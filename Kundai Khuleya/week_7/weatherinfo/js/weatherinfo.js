...
const fetchTemperature = function (event) {
  ...
  $.ajax(`https://api.openweathermap.org/data/2.5/weather?q=${inputVal}&appid=${apiKey}&units=imperial`).done(function(weatherInfo){ //this line is just ajax
    console.log(weatherInfo);
    const temperature = weatherInfo;
    $('#weatherinfo').attr('src', temperature);
    $('#city').html(weatherInfo.name);
    $('#temperature').html(weatherInfo.main.temp);
    console.log($('#city'));
  });
};
